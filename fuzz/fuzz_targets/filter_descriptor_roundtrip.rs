...
use floresta_compact_filters::FilterDescriptor;
use libfuzzer_sys::fuzz_target;

fuzz_target!(|data: &[u8]| {
    // Try to decode from arbitrary bytes. Decoding must never panic.
    if let Ok(dec) = FilterDescriptor::consensus_decode(&mut Cursor::new(data)) {
        // Encode
        let mut buf = Vec::new();
        ...
        let dec2 =
            FilterDescriptor::consensus_decode(&mut Cursor::new(&buf)).expect("decode failed");
