...
/// Maximum age in seconds a chain tip may have for the chain to pass PRESYNC.
///
...
pub struct HeadersSyncState {
    ...
    /// Maximum tip age in seconds for the PRESYNC gate.
    max_tip_age_secs: u32,

    /// Minimum cumulative work the peer's chain must demonstrate before PRESYNC advances.
    /// Mirrors Bitcoin Core's `nMinimumChainWork` per network. The tip-age check is a
    /// secondary condition; work is the primary gate.
    ...
    /// Last header received during PRESYNC, kept to check the tip timestamp.
...
impl HeadersSyncState {
    ...
    ///
    /// `start_height` / `start_hash` identify the last block we already know;
    /// the peer will send headers building on top of it.
    pub fn new(
        start_height: u32,
    ...
    pub fn process_presync(&mut self, headers: &[Header]) -> ProcessingResult {
        ...
        for header in headers {
            ...
            // Validate PoW against the self-declared target only; retarget
            // schedule is not checked here because epoch-boundary timestamps are
            // unavailable until headers reach permanent storage. The minimum-work
            // gate is the load-bearing defense against artificially low difficulty.
        ...
        // Work is the primary gate; tip age is secondary.
        if self.presync_work >= self.minimum_chain_work {
            if let Some(ref tip) = self.presync_last_header {
                let recent_tip = Self::now_secs().saturating_sub(tip.time) <= self.max_tip_age_secs;
                if recent_tip {
...
mod tests {
    ...
    /// Far-past timestamp; tip-age gate rejects chains ending here.
    const ANCIENT_TIMESTAMP: u32 = 1_000_000;
    /// Seconds subtracted from `now()` to produce a "recent" tip; well within DEFAULT_MAX_TIP_AGE.
    const RECENT_TIP_AGO: u32 = 100;
    ...
    #[test]
    fn presync_does_not_accept_headers_before_tip_age_gate() {
        let genesis = BlockHash::all_zeros();
        let mut state = HeadersSyncState::new(0, genesis, DEFAULT_MAX_TIP_AGE, Network::Regtest);

        let chain = make_chain(genesis, ANCIENT_TIMESTAMP, 20);
    ...
    #[test]
    fn presync_transitions_to_redownload_on_recent_tip() {
        let genesis = BlockHash::all_zeros();
        let mut state = HeadersSyncState::new(0, genesis, DEFAULT_MAX_TIP_AGE, Network::Regtest);

        let chain = make_chain(genesis, HeadersSyncState::now_secs() - RECENT_TIP_AGO, 5);
        let result = state.process_presync(&chain);
    ...
    fn redownload_succeeds_with_matching_chain() {
        ...
        // PRESYNC: old-timestamped headers then a recent tip to trigger transition.
        let bulk = make_chain(genesis, ANCIENT_TIMESTAMP, REGTEST_PERIOD);
        let _ = state.process_presync(&bulk);
        let tip_prev = bulk.last().unwrap().block_hash();
        let recent = make_chain(tip_prev, HeadersSyncState::now_secs() - RECENT_TIP_AGO, 5);
        let _ = state.process_presync(&recent);
        assert_eq!(*state.phase(), PresyncPhase::Redownload);

        // REDOWNLOAD: replay the same bulk chain. Hashes are identical so commitments match.
        // On Regtest (min_chain_work = 0), process_all_remaining is set immediately so
        // all headers are released in this batch.
    ...
    fn redownload_releases_all_headers_and_reaches_final() {
        ...
        let mut full_chain = bulk.clone();
        full_chain.extend_from_slice(&recent);
        let total = full_chain.len();

        let result = state.process_redownload(&full_chain);
        assert!(result.success);
        assert_eq!(result.headers_to_accept.len(), total);
    ...
    fn presync_stays_in_presync_below_minimum_work() {
        let genesis = BlockHash::all_zeros();
        let mut state = HeadersSyncState::new(0, genesis, DEFAULT_MAX_TIP_AGE, Network::Bitcoin);

        // Recent tip, but regtest-difficulty headers have negligible work -- far below
        // mainnet nMinimumChainWork. Tip-age would pass alone; this isolates the work gate.
        let chain = make_chain(genesis, HeadersSyncState::now_secs() - RECENT_TIP_AGO, 5);
        let result = state.process_presync(&chain);
    ...
    fn presync_stays_in_presync_below_signet_minimum_work() {
        let genesis = BlockHash::all_zeros();
        let mut state = HeadersSyncState::new(0, genesis, DEFAULT_MAX_TIP_AGE, Network::Signet);

        // Regtest-difficulty headers have negligible work -- far below signet's
        // nMinimumChainWork. Guards against the gate being a no-op on the live network.
        let chain = make_chain(genesis, HeadersSyncState::now_secs() - RECENT_TIP_AGO, 5);
        let result = state.process_presync(&chain);
