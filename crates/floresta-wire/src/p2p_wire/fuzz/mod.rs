    ...
    M: MempoolBase + 'static,
{
    ...
    let remote_sender = tokio::spawn(async move {
        ...
        // Prefix every payload chunk with its fuzzer-controlled length so V2 can exercise
        // sequences of post-handshake packets, not merely one packet per input.
        let mut rest = payload.as_slice();
        while let Some((&len, tail)) = rest.split_first() {
            let (chunk, tail) = tail.split_at(usize::from(len).min(tail.len()));
            let packet = match control & 2 {
                0 => Payload::genuine(chunk),
                _ => Payload::decoy(chunk),
            };
            remote_writer.write(&packet).await?;
            rest = tail;
        }

        let (_, mut writer) = remote_writer.into_inner();
        writer.shutdown().await?;
        Ok::<(), bip324::io::ProtocolError>(())
    });
