...
`floresta-watch-only` is a lightweight, Electrum-first watch-only wallet for the Floresta project. It tracks Bitcoin addresses without requiring private keys, processes blocks in real-time for transaction detection, manages UTXOs with spent-output tracking, and generates Merkle proofs for SPV verification. It supports HD wallets via descriptor-based address derivation, implements the `BlockConsumer` trait to receive block notifications from `floresta-chain`, and provides persistent storage with pluggable database backends.
...
Watch-only wallets allow you to monitor Bitcoin addresses without exposing private keys. This design pattern is essential for:
...
### Balance Monitoring and Auditing

Track incoming and outgoing transactions for accounting, compliance, or notification purposes without the ability to spend funds.

### Hardware Wallet Integration

Provide a backend for hardware wallet interfaces where the signing device never needs to be connected during monitoring operations.

### Server-Side Wallet Infrastructure

Power Electrum servers and other wallet backends that need to serve multiple users' address history and balance data efficiently.
...
let script = ScriptBuf::from_hex("0014...")?;
...
// AddressCache implements BlockConsumer
// When blocks arrive, on_block() is called automatically:
...
let script = ScriptBuf::from_hex("76a91462e907b15cbf27d5425399ebf6f0fb50ebb88f1888ac")?;
