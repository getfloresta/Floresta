...
use super::slip132::generate_descriptor_from_xpub;
use super::slip132::is_xpub_mainnet;
use crate::error::FlorestadError;

/// Parse xpubs into external and internal descriptors.
///
/// Validates xpub matches the target network, rejects private keys (xprv...) and multisig
/// xpubs (pass those as descriptors instead). Generates both external (change=0) and
...
fn parse_xpubs(
    ...
    for key in xpubs {
        ...
        let xpub_is_mainnet = is_xpub_mainnet(key.as_str())?;
        ...
        let external_desc = generate_descriptor_from_xpub(key.as_str(), false)?;
        let internal_desc = generate_descriptor_from_xpub(key.as_str(), true)?;
        descriptors.push(Descriptor::<DescriptorPublicKey>::from_str(&external_desc)?);
        descriptors.push(Descriptor::<DescriptorPublicKey>::from_str(&internal_desc)?);
    }
    Ok(descriptors)
}
