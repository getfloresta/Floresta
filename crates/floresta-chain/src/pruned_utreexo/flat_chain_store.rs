...
use crate::pruned_utreexo::chainstore::InternalError;
...
// NOTE: `PoisonError<MutexGuard<'_, CacheType>>` cannot be boxed as `Box<dyn Error + 'static>`
// due to the borrow lifetime on the guard, the diagnostic information is preserved as the
// `PoisonedLock` enum variant instead
impl From<PoisonError<MutexGuard<'_, CacheType>>> for ChainstoreError {
    fn from(_: PoisonError<MutexGuard<'_, CacheType>>) -> Self {
        InternalError::poisoned_lock()
    }
}
    ...
    impl Index {
        ...
        pub fn new(index: u32) -> Result<Self, ChainstoreError> {
            if index >= Self::FORK_BIT {
                ...
                return Err(InternalError::oversized_index(index));
            }
...
impl FlatChainStore {
    ...
    pub fn new(config: FlatChainStoreConfig) -> Result<Self, ChainstoreError> {
        ...
        let metadata = unsafe {
            metadata
                .as_ref()
                .ok_or_else(InternalError::invalid_metadata_pointer)?
        };

        // check the magic number and version
        if metadata.version > FLAT_CHAINSTORE_VERSION {
            return Err(InternalError::unsupported_schema(metadata.version));
        }

        if metadata.magic != FLAT_CHAINSTORE_MAGIC {
            return Err(InternalError::bad_magic(metadata.magic));
        }
    ...
    unsafe fn add_index_entry(
        ...
        if next_occupancy >= metadata.index_capacity {
            return Err(InternalError::full_index());
        }
        let is_new = unsafe {
            self.block_index
    ...
    unsafe fn get_disk_header(&self, index: Index) -> Result<&HashedDiskHeader, ChainstoreError> {
        ...
        if index >= max_size {
            return Err(InternalError::capacity_exceeded(index, max_size));
        }
    ...
    unsafe fn do_save_height(&mut self, best_block: &BestChain) -> Result<(), ChainstoreError> {
        ...
        if best_block.alternative_tips.len() > 64 {
            return Err(InternalError::too_many_alternative_tips(
...
mod tests {
    ...
    fn test_create_chainstore() {
        ...
        for version in (FLAT_CHAINSTORE_VERSION + 1)..(FLAT_CHAINSTORE_VERSION + 32) {
            ...
            match get_test_chainstore(Some(store_id)) {
                Err(ChainstoreError::Internal(e)) => {
                    let internal = e.downcast_ref::<InternalError>();
                    assert!(
                        matches!(internal, Some(InternalError::UnsupportedSchema(v)) if *v == version),
                        "Expected UnsupportedSchema"
                    );
                }
                Err(e) => {
                    panic!("Should have failed with `UnsupportedSchema`, instead we got {e:?}")
                }
                Ok(_) => panic!("Should have failed with `UnsupportedSchema`, instead we got `Ok`"),
            }
        }
            ...
            .take(32)
        {
            ...
            match get_test_chainstore(Some(store_id)) {
                Err(ChainstoreError::Internal(e)) => {
                    let internal = e.downcast_ref::<InternalError>();
                    assert!(
                        matches!(internal, Some(InternalError::BadMagic(m)) if *m == magic),
                        "Expected BadMagic"
                    );
                }
                Err(e) => panic!("Should have failed with `BadMagic`, instead we got {e:?}"),
                Ok(_) => panic!("Should have failed with `BadMagic`, instead we got `Ok`"),
            }
        }
    }
    ...
    fn test_save_and_retrieve_headers() {
        ...
        unsafe {
            ...
            // exceeds header file capacity, must be CapacityExceeded, not a silent wildcard
            let err = store
                .get_disk_header(Index::new(32_768).unwrap())
                .unwrap_err();
            if let ChainstoreError::Internal(e) = err {
                assert!(
                    matches!(
                        e.downcast_ref::<InternalError>(),
                        Some(InternalError::CapacityExceeded {
                            index: 32768,
                            max_size: 32768
                        })
                    ),
                    "Expected CapacityExceeded for header index exceeding file capacity"
                );
            } else {
                panic!(
                    "Expected ChainstoreError::Internal for header index exceeding file capacity"
                );
            }
        }
        ...
        unsafe {
            ...
            // exceeds fork file capacity, must be CapacityExceeded, not a silent wildcard
            let err = store
            ...
            if let ChainstoreError::Internal(e) = err {
                assert!(
                    ...
                    "Expected CapacityExceeded for fork index exceeding file capacity"
                );
            } else {
                panic!("Expected ChainstoreError::Internal for fork index exceeding file capacity");
            }
        }
    }
    ...
    fn propagates_internal_from_bad_magic() {
        ...
        if let ChainstoreError::Internal(e) = err {
            assert!(
                matches!(
                    e.downcast_ref::<InternalError>(),
                    Some(InternalError::BadMagic(0xBAD0_FACE))
                ),
                "Expected specific BadMagic internal error"
            );
        } else {
            panic!("Expected ChainstoreError::Internal error");
        }
    }
    ...
    fn propagates_internal_from_unsupported_schema() {
        ...
        if let ChainstoreError::Internal(e) = err {
            assert!(
                matches!(
                    e.downcast_ref::<InternalError>(),
                    Some(InternalError::UnsupportedSchema(_))
                ),
                "Expected specific UnsupportedSchema internal error"
            );
        } else {
            panic!("Expected ChainstoreError::Internal error");
        }
    ...
    fn propagates_internal_from_oversized_index() {
        ...
        if let ChainstoreError::Internal(e) = err {
            assert!(
                matches!(
                    e.downcast_ref::<InternalError>(),
                    Some(InternalError::OversizedIndex(0x8000_0000))
                ),
                "Expected specific OversizedIndex internal error"
            );
        } else {
            panic!("Expected ChainstoreError::Internal error");
        }
    ...
    fn propagates_internal_from_capacity_exceeded() {
        ...
        if let ChainstoreError::Internal(e) = err {
            assert!(
                ...
                "Expected specific CapacityExceeded internal error"
            );
        } else {
            panic!("Expected ChainstoreError::Internal error");
        }
