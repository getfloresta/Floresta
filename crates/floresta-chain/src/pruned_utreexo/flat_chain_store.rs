...
impl ChainStore for FlatChainStore {
    ...
    fn save_block_fee_rate(&mut self, height: u32, fee_rate: FeeRate) -> Result<(), Self::Error> {
        ...
        self.fee_rate_file
            .write_all(&fee_rate.to_sat_per_kwu().to_le_bytes())?;
        Ok(())
    }
    ...
    fn get_block_fee_rate(&mut self, height: u32) -> Result<Option<FeeRate>, Self::Error> {
        ...
        Ok(Some(FeeRate::from_sat_per_kwu(u64::from_le_bytes(buf))))
    }
}
...
mod tests {
    ...
    fn test_fee_rate_save_and_get() {
        ...
        // save and get
        store.save_block_fee_rate(0, fr(12345)).unwrap();
        store.save_block_fee_rate(1, fr(67890)).unwrap();
        store.save_block_fee_rate(100, fr(u64::MAX)).unwrap();

        assert_eq!(store.get_block_fee_rate(0).unwrap(), Some(fr(12345)));
        assert_eq!(store.get_block_fee_rate(1).unwrap(), Some(fr(67890)));
        assert_eq!(store.get_block_fee_rate(100).unwrap(), Some(fr(u64::MAX)));
    }
    ...
    fn test_fee_rate_prune_window() {
        ...
        store.save_block_fee_rate(h1, fr(100)).unwrap();
        assert_eq!(store.get_block_fee_rate(h1).unwrap(), Some(fr(100)));

        // Case 2: at window boundary → None (tip - h2 = W >= W)
        let h2 = W;
        store.save_block_fee_rate(h2, fr(200)).unwrap();
        assert_eq!(store.get_block_fee_rate(h2).unwrap(), None);

        // Case 3: future block → None
        let h3 = tip + 1;
        store.save_block_fee_rate(h3, fr(300)).unwrap();
    ...
    fn test_fee_rate_overwrite() {
        ...
        store.save_block_fee_rate(5, fr(111)).unwrap();
        store.save_block_fee_rate(5, fr(222)).unwrap();
        assert_eq!(store.get_block_fee_rate(5).unwrap(), Some(fr(222)));
    }
