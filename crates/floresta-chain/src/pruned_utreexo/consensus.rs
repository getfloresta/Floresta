...
impl Consensus {
    ...
    ///
    /// Before CSV activation this is the block header time. After activation it
    /// is the previous block's Median Time Past (MTP).
    pub(crate) fn block_lock_time_cutoff<E>(
        ...
        if height >= csv_activation_height {
    ...
    ///
    /// A zero lock time is final. Otherwise, height locks must be below
    /// `height` and time locks below `lock_time_cutoff`. If every input has
    /// final sequence, nLockTime is disabled and the transaction is final.
...
mod tests {
    ...
    const UNUSED_HEIGHT: u32 = 0;
    const UNUSED_LOCK_TIME_CUTOFF: u32 = 0;
