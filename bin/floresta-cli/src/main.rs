...
fn do_request(cmd: &Cli, client: Client) -> anyhow::Result<String> {
    Ok(match cmd.methods.clone() {
        ...
        Methods::VerifyUtxoChainTipInclusionProof { proof } => {
            serde_json::to_string_pretty(&client.verify_utxo_chain_tip_inclusion_proof(proof)?)?
        }
    })
}
...
pub enum Methods {
    ...
    VerifyUtxoChainTipInclusionProof { proof: String },
}
