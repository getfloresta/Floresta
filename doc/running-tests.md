...
We provide a single helper script [run_functional.sh](https://github.com/getfloresta/Floresta/blob/master/tests/run_functional.sh) that checks build dependencies, builds all required binaries, and runs the tests.
...
```bash
UTREEXO_REVISION=0.1.0 ./tests/run_functional.sh
```

##### Bitcoin-core

By default, the script will obtain a runnable `bitcoind` binary in one of three exclusive ways. The default Bitcoin Core version is `30.2`, but you can override this by setting the `BITCOIN_REVISION` environment variable. The three methods are:
...
```bash
./tests/run_functional.sh --build --release --preserve-data-dir
```
