...
```bash
floresta-cli verifyutxochaintipinclusionproof <proof>
```
...
* `JsonRpcError::Decode` - The proof is too short, has malformed varints, contains truncated data, or was generated at a different block height than the current chain tip (stale proof).

## Notes

- The proof must be generated at the **exact same block height** as the current chain tip. Stale proofs (generated when the chain was at a different height) will result in an error with a message indicating the mismatch.
