...
from test_framework.rpc.base import assert_rpc_error
...
class TestGetAddedNodeInfo:
    ...
    def test_get_added_node_info_connected(
        ...
        """
        shared_florestad_node.rpc.addnode(
            shared_bitcoind_node.p2p_url, "add", v2transport=True
        )
        shared_node_manager.wait_for_peers_connections(
            shared_florestad_node, shared_bitcoind_node
        )
    ...
    def test_get_added_node_info_multiple_nodes(
        ...
        shared_florestad_node.rpc.addnode(bitcoind_a.p2p_url, "add", v2transport=False)
        shared_florestad_node.rpc.addnode(bitcoind_b.p2p_url, "add", v2transport=False)

        shared_node_manager.wait_for_peers_connections(
            shared_florestad_node, bitcoind_a
        )
        shared_node_manager.wait_for_peers_connections(
            shared_florestad_node, bitcoind_b
        )
