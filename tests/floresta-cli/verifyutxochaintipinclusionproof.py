...
class TestVerifyProofOnSharedChain:
    ...
    def test_invalid_verbosity(self):
        ...
        resp = make_request(
            self.florestad, "verifyutxochaintipinclusionproof", [proof_hex, 2]
        )
        assert_rpc_error(resp, expected_rpcerror_code=JSONRPC_ERRCODE_INVALID_PARAMS)
    ...
    def test_unknown_blockhash(self):
        ...
        assert_rpc_error(resp, expected_rpcerror_code=JSONRPC_ERRCODE_BLOCK_NOT_FOUND)
...
class TestVerifyProofInvalidInput:
    ...
    def test_invalid_proof(self):
        ...
        garbage_proof = "aa" * 32
        resp = make_request(
            self.florestad, "verifyutxochaintipinclusionproof", [garbage_proof]
        )
        assert_rpc_error(resp, expected_rpcerror_code=JSONRPC_ERRCODE_INVALID_PARAMS)
    ...
    def test_invalid_hex(self):
        ...
        resp = make_request(
            self.florestad, "verifyutxochaintipinclusionproof", ["not_valid_hex!!"]
        )
        assert_rpc_error(resp, expected_rpcerror_code=JSONRPC_ERRCODE_INVALID_PARAMS)
