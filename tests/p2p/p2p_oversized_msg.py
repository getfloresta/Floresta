...
class TestP2pOversizedMessages(FlorestaTestFramework):
    ...
    def run_test(self):
        ...
        for msg_class in selected:
            self.log(f"Testing {msg_class.__name__}")
            self.connect_p2p()

            self.log(f"Testing {msg_class.__name__} with version v1")
            ...
            self.log(f"Testing {msg_class.__name__} with version v2")
            msg = self.create_msg_random(
                msgtype=msg_class.msgtype, size=16777216 + 1
            )  # 16 MiB + 1 byte
            self.p2p_conn_v2.send_without_ping(msg)
            self.check_disconnection(self.p2p_conn_v2)

    def connect_p2p(self, expected_peer_count: int = 2):
        """Establish a P2P connection to the Floresta node."""
        self.log("Connecting to interface P2P...")
        self.log("Connecting with v1 P2P protocol...")
        self.p2p_conn_v1 = self.add_p2p_connection_default(
            node=self.florestad, p2p_idx=0, supports_v2_p2p=False
        )
        wait_until(
            predicate=lambda: self.floresta_has_peer_count(
                expected_peer_count=expected_peer_count - 1
            ),
            error_msg="Floresta node did not connect as expected",
        )

        self.log("Connecting with v2 P2P protocol...")
        self.p2p_conn_v2 = self.add_p2p_connection_default(
            node=self.florestad, p2p_idx=0, supports_v2_p2p=True
        )
        wait_until(
            predicate=lambda: self.floresta_has_peer_count(
