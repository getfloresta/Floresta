...
class TestP2pSpamMessages(FlorestaTestFramework):
    ...
    def run_test(self):
        ...
        for version in self.versions:
            for msg_class in self.message_classes:
                self.log(f"Testing {msg_class.msgtype} with version {version}")
                self.connect_p2p(supports_v2_p2p=(version == "v2"))

                self.send_spam_p2p_messages(msg=msg_class, p2p_conn=self.p2p_conn)
    ...
    def floresta_has_peer_count(self, expected_peer_count: int) -> bool:
