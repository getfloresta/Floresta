...
def _start_florestad(node_manager, extra_args, load_cookie):
    """
    Register and launch a florestad node bypassing `Node.start()` so tests can
    drive scenarios where no `.cookie` file is written (e.g. -rpcuser auth).
    Falls back to the regular start path when `load_cookie` is True.
    """
